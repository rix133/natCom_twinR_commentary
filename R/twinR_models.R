...
#' @export
#'
fit_totalbirths <- function(mother_level_data, when_twinner = "allbirths", with_age = FALSE, timeout = Inf, verbose = TRUE, extra_vars="") {
  
  if (when_twinner == "allbirths") {
    if (with_age) {
      formula <- paste0("births_total ~ 1 + twinner * scale(AFB) ", extra_vars)
    } else {
      formula <- paste0("births_total ~ 1 + twinner ", extra_vars)
    }
  } else if (when_twinner == "firstbirth") {
    if (with_age) {
      formula <- paste0("births_total ~ 1 + first_twinner * scale(AFB) ", extra_vars)
    } else {
      formula <- paste0("births_total ~ 1 + first_twinner ", extra_vars)
    }
...
#' @export
#'
fit_twinner.allbirths <- function(mother_level_data, with_age = FALSE, timeout = Inf, verbose = TRUE, extra_vars="") {
  
  if (with_age) {
    formula <- paste0("twinner ~ 1 + births_total*scale(AFB) ", extra_vars)
  } else {
    formula <- paste0("twinner ~ 1 + births_total ", extra_vars)
  }
...
#' @export
#'
fit_twinner.firstbirth <- function(mother_level_data, timeout = Inf, verbose = TRUE, extra_vars="") {
  
  formula <- paste0("first_twinner ~ 1 + births_total ", extra_vars)
...
#' @export
#'
fit_twinning.binomial <- function(mother_level_data, timeout = Inf, verbose = TRUE, extra_vars="") {
  
  formula <- paste0("cbind(twin_total, singleton_total) ~ 1 + births_total ", extra_vars)
...
#' @export
#'
fit_twinning.binomial_with_pop_as_fixed <- function(mother_level_data, timeout = Inf, verbose = TRUE, extra_vars="") {
...
#' @export
#'
fit_twinning.binomial_with_pop_interaction <- function(mother_level_data, timeout = Inf, verbose = TRUE, extra_vars="") {
...
#' @export
#'
fit_AFB <- function(mother_level_data, timeout = Inf, verbose = TRUE, extra_vars="") {
  
  formula <- paste0("AFB ~ 1 + twinner * births_total_fac ", extra_vars)
...
#' @export
#'
fit_PP <- function(birth_level_data, poly_order = NA, twin_as.predictor = TRUE, max_order = 6L, timeout = Inf, verbose = TRUE, extra_vars="", .simpler_fit = NULL) {
  ...
  if (poly_order == 0) {
    formula <- paste0("PP ~ 1 + (1|maternal_id) ", extra_vars)
  } else {
    formula <- paste0("PP ~ 1 + poly(cbind(age, parity), ", poly_order, ") + (1|maternal_id) ", extra_vars)
  }
...
#' @export
#'
fit_IBI <- function(birth_level_data, poly_order = NA, twin_as.predictor = TRUE,  max_order = 6L, timeout = Inf, verbose = TRUE, extra_vars="", .simpler_fit = NULL) {
  ...
  if (poly_order == 0) {
    formula <- paste0("IBI ~ 1 + (1|maternal_id) ", extra_vars)
  } else {
    formula <- paste0("IBI ~ 1 + poly(cbind(age, parity), ", poly_order, ") + (1|maternal_id) ", extra_vars)
  }
...
#' @export
#'
fit_twinning.binary <- function(birth_level_data, poly_order = NA, maternal_ID_as.predictor = TRUE, max_order = 6L, timeout = Inf, verbose = TRUE, extra_vars="", .simpler_fit = NULL) {
  ...
  if (maternal_ID_as.predictor) {
    formula <- paste(formula, " + (1|maternal_id) ", extra_vars)
  } else {
    formula <- paste(formula, " ", extra_vars)
  }
...
#' @export
#'
fit_life_histories <- function(scenario, birth_level_data, max_order = 6L, timeout = Inf, verbose = TRUE, extra_vars="") {
  ...
  fit_PP <- fit_PP(birth_level_data = birth_level_data, poly_order = NA, twin_as.predictor = grepl("P", scenario), max_order = max_order, timeout = timeout, verbose = verbose, extra_vars=extra_vars)
  
  ## fit IBI with or without twin as predictor:
  fit_IBI <- fit_IBI(birth_level_data = birth_level_data, poly_order = NA, twin_as.predictor = grepl("I", scenario), max_order = max_order, timeout = timeout, verbose = verbose, extra_vars=extra_vars)
  ...
  fit_twinning.binary <- fit_twinning.binary(birth_level_data = birth_level_data,
                                             ...
                                             max_order = max_order,  timeout = timeout, verbose = verbose, extra_vars=extra_vars)
...
#' @export
#'
compute_slope_from_birth.level.data = function(birth_level_data, timeout = Inf, verbose = TRUE, extra_vars="") {
  ...
  ## fit the model:
  fit_twinning.binomial <- fit_twinning.binomial(mother_level_data = mother_level_data, timeout = timeout, verbose = verbose, extra_vars=extra_vars)
