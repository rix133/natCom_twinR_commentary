...
## Data Import 8 European Populations
...
all_tbls$Populations  <- rep(c("8 European", "Estonia"),each=2)
...
fig2_ext_orig <- ggplot() +  
  geom_line(data=fig2_EE_plot_data,
    ...
    geom_line(data=fig2_orig_plot_data,
             aes(y = estimate,  x=births_total, color="8 European"),  size = 1) +
    stat_summary(data=dmm_orig[dmm_orig$births_total <19, ],
                 aes(x=births_total, y=prob_twin,
                     color="8 European", fill="8 European"),
                 alpha=0.5,
                 fun.data=mean_se) +
...
fig2a <- draw_fig_4C(fig4C_orig_plot_data) + 
  labs(subtitle = "(A) 8 European pop., all births") 
...
fig2c <- draw_fig_4C(fig4C_orig_nl_plot_data, y_lims = c(0.01, 0.035)) + 
  labs(subtitle = "(C) 8 European pop., without last birth") 
